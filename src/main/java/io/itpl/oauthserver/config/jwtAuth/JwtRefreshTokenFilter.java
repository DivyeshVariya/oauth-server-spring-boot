...
public class JwtRefreshTokenFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(HttpServletRequest request,
    ...
    try {
      ...
      log.info("Before the if statement in JWTRefreshTokenFilter");
      if (!userName.isEmpty() && SecurityContextHolder
              .getContext()
              .getAuthentication()==null) {
        log.info("Inside the if statement in JWTRefreshTokenFilter");
        ...
        var isRefreshTokenValidInDatabase = refreshTokenRepo
                ...
                .orElse(false);
        log.info("RefreshToken valid in database: " + isRefreshTokenValidInDatabase);
        if (Boolean.FALSE.equals(isRefreshTokenValidInDatabase)) {
          throw new IllegalStateException("RefreshToken invalid");
        }
        UserDetails userDetails = jwtTokenUtils.userDetails(userName);
        if (jwtTokenUtils.isTokenValid(jwtRefreshToken, userDetails) && isRefreshTokenValidInDatabase) {
          log.info("Valid RefreshToken inside if statement");
          ...
          log.info("After setting SecurityContext in JWTRefreshTokenFilter");
        }
        log.info("After the if statement in JWTRefreshTokenFilter");
      }
